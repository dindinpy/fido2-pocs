...
def generate_challenge(challenge_len=CHALLENGE_DEFAULT_BYTE_LEN, auth = False):
    ...
    challenge_bytes = _custom_challenge() if auth else os.urandom(challenge_len)
...
def _custom_challenge():
    print("[INFO] _custom_challenge()")
    transaction = {
    ...
    jt = json.dumps(out)
    print("[INFO] transaction json ", jt)
    # hash = hashlib.sha256(jt.encode()).hexdigest()
    trans_digest = hashlib.sha256(jt.encode()).digest()
    return trans_digest