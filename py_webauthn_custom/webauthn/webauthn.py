...
class WebAuthnRegistrationResponse(object):
    ...
    def _verify_attestation_statement(self, fmt, att_stmt, auth_data,
        ...
        '''
        # print("_verify_attestation()  auth_data ", auth_data)
        ...
        print("_verify_attestation() aaguid ", aaguid)
        if fmt == AT_FMT_FIDO_U2F:
            ...
            # print("_verify_attestation() att_stmt", att_stmt)
            print("_verify_attestation() alg ", alg)
            if 'x5c' in att_stmt:
                # Step 2.
                #
...
class WebAuthnAssertionResponse(object):
    ...
    def verify(self):
        try:
            ...
            print("verify() step4 signature ", self.assertion_response.get('signature'))
            print("verify() step4 unhexlified sig ", sig)
...
try:
    ...
    print("[LIB] import of ed25519 successful")
except Exception as e:
    print("[ERROR_LIB] import of ed25519 failed ", e)
...
def _load_cose_public_key(key_bytes):
    ...
    if alg == COSE_ALG_ES256:
        ...
        print("[LIB] len(E_key) -2 ", len(cose_public_key[E_KEY]))
        print("[LIB] len(N_key) -1 ", cose_public_key[N_KEY])

        if cose_public_key[N_KEY] != 6:
            raise COSEKeyException("Unsupported elliptic curve")
        print("[LIB] using curve ed25519 to verify signature")
...
def _verify_signature(public_key, alg, data, signature):
    if alg == COSE_ALG_ES256:
        ...
        print("[LIB] verifying signature")
        public_key.verify(signature, data)
        print("[LIB] signature verified")
    else:
        raise NotImplementedError()
